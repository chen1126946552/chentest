...
let exp = {
	...
	Case_02: 'peterangel536@gmail.com',  // 授权GA账号，并截图，再删除新授权账号
	Case_03: 'guy.geeraedts@ptengine.com',  // 授权Mailchimp账号，并截图，再删除新授权账号
	Case_04: 'bin.wu@ptengine.com',  // 授权Mailchimp账号，并截图，再删除新授权账号
};
...
(async () => {
	...
	try {
		...
		let Case_02_Act = await page.$eval('.connection-list__col-name > span.connection-list__name', el => el.innerText);
		...
		// Case_03
		deleteNode(page)
		// const Case_03 = 'Case_03: Delete new GA account. Capture [dd_ui_Datasource_Delete_New_GA.jpg].';
		// log(Case_03);

		// const datasourceListGA = await page.$$('.connection-list__col-tool > span > span > span')
		// await datasourceListGA[0].click()
		// await page.waitFor(2000)
	
		// const removebuttoneGA = await page.$('body > ul > li')
		// await removebuttoneGA.click()

		// await page.waitForSelector('body > div.pt-confirm-backdrop > div > section');
		// let Case_03_Act = await page.$eval('body > div.pt-confirm-backdrop > div > section', el => el.innerText);
		// let Case_03_Res = 'Case_03_Res: ' + cps(exp.Case_03, Case_03_Act);
		// log(Case_03_Res);
		// await page.waitFor(3000);
		// await page.screenshot({
		// 	path: './pic/dd_ui_Datasource_Delete_New_GA.jpg',
		// 	type: 'png',
		// 	clip: {
		// 		x:0, y:0, width:1366, height:documentSize.height
		// 	}
		// 	});
		// const Conn_Delete_ButtonGA = await page.$('body > div.pt-confirm-backdrop > div > footer > button.pt-ui-button.pt-ui-button--primary.pt-ui-button--type-default.pt-ui-button--size-big');
		// await Conn_Delete_ButtonGA.click();
		// await page.waitFor(2000);
    	...
    	// 点击允许授权
    	// const approveAccess = await pageMailchimp.waitFor('#submit_approve_access');
    	// await approveAccess.click()
    	// await pageMailchimp.waitFor(2000)

		await page.waitForSelector('.connection-list__col-name > span.connection-list__name');
		let Case_03_Act = await page.$eval('.connection-list__col-name > span.connection-list__name', el => el.innerText);
		let Case_03_Res = 'Case_03_Res: ' + cps(exp.Case_03, Case_03_Act);
		log(Case_03_Res);
		...
		/*
		// Case_05
		const Case_05 = 'Case_05: Delete new Mailchimp account. Capture [dd_ui_Datasource_Delete_New_Mailchimp.jpg].';
		log(Case_05);

		const datasourceListMailchmip = await page.$$('.connection-list__col-tool > span > span > span')
		await datasourceListMailchmip[0].click()
		await page.waitFor(2000)
	
		const removebuttoneMailchmip = await page.$('body > ul > li')
		await removebuttoneMailchmip.click()

		await page.waitForSelector('body > div.pt-confirm-backdrop > div > section');
		let Case_05_Act = await page.$eval('body > div.pt-confirm-backdrop > div > section', el => el.innerText);
		...
		const Conn_Delete_ButtonMailchmip = await page.$('body > div.pt-confirm-backdrop > div > footer > button.pt-ui-button.pt-ui-button--primary.pt-ui-button--type-default.pt-ui-button--size-big');
		await Conn_Delete_ButtonMailchmip.click();
		await page.waitFor(2000);
		*/
		/*
		// Case_06
		...
		await page.waitForSelector('.connection-list__col-name > span.connection-list__name');
		let Case_04_Act = await page.$eval('.connection-list__col-name > span.connection-list__name', el => el.innerText);
		...
		*/
		...
		/*
		// Case_07
		const Case_07 = 'Case_07: Delete new Facebookpages account. Capture [dd_ui_Datasource_Delete_New_Facebookpages.jpg].';
		log(Case_07);

		const datasourceListFacebookpages = await page.$$('.connection-list__col-tool > span > span > span')
		await datasourceListFacebookpages[0].click()
		await page.waitFor(2000)
	
		const removeFacebookpages = await page.$('body > ul > li')
		await removeFacebookpages.click()

		await page.waitForSelector('body > div.pt-confirm-backdrop > div > section');
		let Case_07_Act = await page.$eval('body > div.pt-confirm-backdrop > div > section', el => el.innerText);
		let Case_07_Res = 'Case_07_Res: ' + cps(exp.Case_07, Case_07_Act);
		log(Case_07_Res);
		await page.waitFor(3000);
		await page.screenshot({
			path: './pic/dd_ui_Datasource_Delete_New_Facebookpages.jpg',
			type: 'png',
			clip: {
				x:0, y:0, width:1366, height:documentSize.height
			}
			});
		const Conn_Delete_ButtonFacebookpages = await page.$('body > div.pt-confirm-backdrop > div > footer > button.pt-ui-button.pt-ui-button--primary.pt-ui-button--type-default.pt-ui-button--size-big');
		await Conn_Delete_ButtonFacebookpages.click();
		await page.waitFor(2000);
		*/

		// log
		const log_server_1 = 'log_server_1: 【AT_DD_Online_UI_SAAS_Auth】 runs Finished.';
		log(log_server_1);
		...
		if (e instanceof TimeoutError) {
			console.error(e);
			const log_block = 'Log_Block: Service unexpected termination! 【AT_DD_Online_UI_SAAS_Auth】 runs Error!';
			log(log_block);
