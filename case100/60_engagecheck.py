...
#    sql = 'select DISTINCT sid from pt_engage_base WHERE `status`=1 and deleted=0;'
    sql = "select sid from pt_engage_base where sid='2b1ab5d0'"
...
def getdata(profileId, startdate, enddate, dimensions, meterics,):
    # 事件请求接口
    url2 = "https://ptesthquery.ptmind.com/wa/dmevent/v1_0/data?"
    headers = {
...
def engage_data_sql(sql):
    # 打开数据库连接
    db = pymysql.connect(host='139.220.242.37',
                         port=23306,
                         user='qa_auto',
                         passwd='qa_auto',
                         db='qa_auto_ptengine',
...
if __name__ == "__main__":
    ...
    yesterday = datetime.date.today() + datetime.timedelta(-30)
    engageView = 0
    print(today)
    for i in sidlist:
        s = getdata(i['sid'],yesterday, today, 'hit::pt:engageId,hit::pt:engageName', 'pt:engageView,pt:engageCTR,pt:engageCloseRate,pt:perExit,pt:conversions,pt:conversionRate')
        d = eval(s[1])
        rows = d['rows']
        if len(rows) == 0:
